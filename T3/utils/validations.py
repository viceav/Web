...
def validate_text(text):
    if not text:
        flash('Falta ingresar el Mensaje')
        return False
    text_valid = len(text.strip()) >= 5
    if not text_valid:
        flash('Ingrese un Mensaje válido')
    return text_valid


def validateCommentName(name):
    if not name:
        flash('Falta ingresar el Nombre')
        return False
    lenghtValid = 80 >= len(name.strip()) >= 3
    if not lenghtValid:
        flash('Ingrese un Nombre válido')
    return lenghtValid


def validateComment(name, comment):
    NameIsValid = validateCommentName(name)
    CommentIsValid = validate_text(comment)
    return NameIsValid and CommentIsValid
